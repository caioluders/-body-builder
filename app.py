import os, argparse, functools, datetime , string , operator , json , re
...
""" % ( f[0] ,f[0], f[1] ,( datetime.datetime.fromtimestamp(int(f[2])).strftime("%d/%m/%Y") if f[2].isdigit() else f[2] ) )
	...
	if len(files_dated) > 0 :
	    ...
	    latest += open(os.path.join(path,files_dated[0][0]), encoding="utf-8" ).read()
	    if files_dated[0][0][-3:] == "txt" :
	      latest = latest.replace("\n","<br>")
	else :
	    latest = ""

	html_result = string.Template( template ).substitute( { "posts_table" : table_html ,
	"blog_title": cfg["title"] , "latest_entry":latest} )
...
def main(args) :
	...
	for root, dirs, files in os.walk(path) :
		files = [f for f in files if not f[0] == '.' and f not in config["ignore"] ]
